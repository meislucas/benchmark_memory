...
def pandas_benchmark(csv_files: [str]) -> [(str, str, float)]:
    """
    Benchmark memory usage for reading CSV files using pandas.
    ...
    for csv_file in csv_files:
        ...
        try:
            # Reading
            read_mem = benchmark_memory(pd.read_csv, csv_file)
            logging.info(f"Read memory: {read_mem}")

            mem_usages.append((csv_file, 'pandas', read_mem))
